...
Nesse tutorial, você aprenderá a criar um tema responsivo do [WordPress](https://br.wordpress.org/) usando o [Bootstrap]({{% ref "2015-07-02-bootstrap" %}}). Com um tema responsivo, seu *blog* ou *site* com WordPress aparecerá bem na grande variedade de dispositivos (computadores, *smartphones* e *tablets*) e navegadores existentes.

Essa é a segunda parte do tutorial. Se você deseja acessar a primeira parte, então [clique aqui]({{% ref "2015-08-06-wordpress-bootstrap" %}}). Senão, continue lendo.
...
Em seguida, vamos criar um [*layout* de duas colunas com o Bootstrap]({{% ref "2015-07-02-bootstrap" %}}). Modifique o arquivo `page.php` para incluir uma `div.row` com uma `div.col-md-8` e uma `div.col-md-4`. Vamos utilizar a `div.col-md-8` para o conteúdo da página e a `div.col-md-4` para a barra lateral:
